...
with tab_list[0]:
    ...
    if not df.empty:
        ...
        3. **เซิร์ฟเวอร์ถูกบล็อกชั่วคราว (Rate Limit):** ลองเปลี่ยนเป็นหุ้นตัวอื่น (เช่น AAPL) หากยังดึงไม่ได้เหมือนกัน แสดงว่าติดข้อจำกัดของ Yahoo ให้รอสักพักแล้วลองใหม่ครับ
        """)
...
if st.session_state["logged_in"]:
    ...
    with tab_list[1]:
        ...
        if not ed_l.equals(st.session_state.trade_ledger):
            ...
            st.session_state.trade_ledger = sorted_ed_l
            st.rerun()
