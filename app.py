...
if not df.empty:
    ...
    with col_plan:
        ...
        if buy_price > 0:
            ...
            # 🌟 จุดตัดขาดทุน (คำนวณจากต้นทุน -8%)
            stop_8pct = buy_price * 0.92
            st.markdown("---")
            st.error(f"**🛡️ จุดตัดขาดทุน (Stop Loss):** ${stop_8pct:.2f}\n\n*กฎเหล็ก: หากราคาหลุดเส้นนี้ ต้องตัดใจขายเพื่อป้องกันพอร์ตพังค่ะ (-8% จากต้นทุน)*")
        else:
            # 🌟 จุดตัดขาดทุนเชิงเทคนิค (EMA 50)
            sl_price = df['EMA_50'].iloc[-1] * 0.99
            st.markdown("---")
            st.error(f"**🛡️ จุดตัดขาดทุน (Stop Loss):** ${sl_price:.2f}\n\n*หนีตายเมื่อหลุดเส้น EMA 50 (เผื่อสัญญาณหลอก 1%) เพื่อลดความเสี่ยงค่ะ*")
